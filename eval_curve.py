...
tf.app.flags.DEFINE_float('min_area', 16*16/4/4, 'filter min area')
...
def resize_image_range(im, min_side_len=FLAGS.min_side_len, max_side_len=FLAGS.max_side_len, re_ratio=1.0):
    '''
    resize image to a size multiple of 32 which is required by the network
    :param im: the resized image
    :param max_side_len: limit of max image size to avoid out of memory in gpu
            ...
            for im_fn in im_fn_list:
                ...
                with open(res_file, 'w') as f:     
                    for contour in mask_contours:
                        ...
                        for p_i in range(contour.shape[0]-1,0,-1):
                            contour_line = contour_line + str(contour[p_i,0,1]) + "," + str(contour[p_i,0,0]) + ","
                        contour_line = contour_line + str(contour[0,0,1]) + "," + str(contour[0,0,0]) + "\n"
